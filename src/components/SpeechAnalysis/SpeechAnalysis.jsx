...
const SpeechAnalysisDashboard = ({ data, isLoading }) => {
  ...
  const sentiment = getSentimentLabel(data.sentiment.compound);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Your speech has been analyzed across multiple dimensions
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600">Topic Match</span>
              <Sparkles className="w-5 h-5 text-purple-500" />
            </div>
            <div className="text-2xl font-bold text-gray-900">
              {Math.round(data.semantic_similarity * 100)}%
            </div>
            <div
              className={`text-xs font-medium mt-2 px-2 py-1 rounded-full inline-block ${getScoreColor(data.semantic_similarity)}`}
            >
              {getScoreLabel(data.semantic_similarity)}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div
              className={`text-xs font-medium mt-2 px-2 py-1 rounded-full inline-block ${getScoreColor(data.vocabulary_richness)}`}
            >
              {getScoreLabel(data.vocabulary_richness)}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div
              className={`text-xs font-medium mt-2 px-2 py-1 rounded-full inline-block ${sentiment.color}`}
            >
              {Math.round(data.sentiment.compound * 100)}% confidence
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="text-xs text-gray-500 mt-2">
              {data.grammar_issues.length === 0 ? "Perfect!" : "Found issues"}
            </div>
          </div>
        </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-gray-900">
                        {data.sentence_metrics.complexity.flesch_reading_ease.toFixed(
                          1,
                        )}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <span className="font-medium text-gray-900">
                        {data.sentence_metrics.complexity.flesch_kincaid_grade.toFixed(
                          1,
                        )}
                      </span>
                    </div>
              ...
              <div className="space-y-4">
                {data.grammar_issues.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600">
                      Your speech is grammatically correct.
                    </p>
                  </div>
                ) : (
